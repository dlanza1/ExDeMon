...
public class CustomAnomalyLikelihood extends Anomaly implements Persistable {

	private static final long serialVersionUID = 8985346733060793386L;

	private static final Logger LOG = LoggerFactory.getLogger(CustomAnomalyLikelihood.class);
    ...
    public CustomAnomalyLikelihood(boolean useMovingAvg, int windowSize, boolean isWeighted, int claLearningPeriod, int estimationSamples) {
    ...
    public double anomalyProbability(double value, double anomalyScore, DateTime timestamp) {
        ...
        if(historicalScores.size() < probationaryPeriod) {
            ...
            CustomAnomalyLikelihoodMetrics metrics = updateAnomalyLikelihoods(Arrays.asList(dataPoint), this.distribution);
    ...
    public CustomAnomalyLikelihoodMetrics updateAnomalyLikelihoods(List<Sample> anomalyScores, NamedTuple params) {
    ...
    private CustomAnomalyLikelihoodMetrics estimateAnomalyLikelihoods(List<Sample> anomalyScores, int averagingWindow, int skipRecords) {
    	if(anomalyScores.size() == 0) {
            throw new IllegalArgumentException("Must have at least one anomaly score.");
        }
        ...
        return new CustomAnomalyLikelihoodMetrics(filteredLikelihoods, records, params); 
	}
	...
	public static class AnomalyParams extends NamedTuple implements Persistable {

		private static final long serialVersionUID = 526861267905350185L;
