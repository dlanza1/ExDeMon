...
public class Actuators {
	...
	private static final Cache<Map<String, Actuator>> actuators = new Cache<Map<String, Actuator>>() {
		...
		protected Map<String, Actuator> load() throws Exception {
	        Properties properties = Properties.getCache().get().getSubset("actuators");
	        
	        Set<String> actuatorIDs = properties.getIDs();
	        Map<String, Actuator> actuators = actuatorIDs.stream()
	        		.map(id -> {
	        		   try {
	        		...
	        		.filter(actuator -> actuator != null)
	        		.collect(Collectors.toMap(Actuator::getId, a -> a));
