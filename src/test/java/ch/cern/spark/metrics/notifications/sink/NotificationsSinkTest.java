...
public class NotificationsSinkTest {
    ...
    public void filter() throws ConfigurationException {
        ...
        Notification notif = NotificationTest.DUMMY;
        
        notif.setSink_ids(new HashSet<>(Arrays.asList("aa", "bb")));
        assertFalse(sink.shouldBeSink(notif));
        
        notif.setSink_ids(new HashSet<>(Arrays.asList("ALL")));
        assertTrue(sink.shouldBeSink(notif));
        
        notif.setSink_ids(new HashSet<>(Arrays.asList("test")));
        assertTrue(sink.shouldBeSink(notif));
        
        notif.setSink_ids(new HashSet<>(Arrays.asList("aa", "test")));
        assertTrue(sink.shouldBeSink(notif));
        
        notif.setSink_ids(new HashSet<>(Arrays.asList("aa", "ALL")));
        assertTrue(sink.shouldBeSink(notif));
    }
