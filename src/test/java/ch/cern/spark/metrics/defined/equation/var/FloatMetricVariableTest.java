...
public class FloatMetricVariableTest {
    ...
    @Test
    public void ignorePeriod() throws ConfigurationException, ComputationException {
        Properties properties = new Properties();
        properties.setProperty("ignore", "1m");
        variable.config(properties);

        store.add(new FloatValue(10), now);
        
        Value result = variable.compute(store, now);
        assertTrue(result.getAsException().isPresent());
        
        result = variable.compute(store, now.plus(Duration.ofSeconds(60)));
        assertTrue(result.getAsException().isPresent());
        
        result = variable.compute(store, now.plus(Duration.ofSeconds(61)));
        assertTrue(result.getAsFloat().isPresent());
        assertEquals(10, result.getAsFloat().get(), 0);
        
        store.add(new FloatValue(20), now.plus(Duration.ofSeconds(120)));
        
        result = variable.compute(store, now.plus(Duration.ofSeconds(120)));
        assertTrue(result.getAsFloat().isPresent());
        assertEquals(10, result.getAsFloat().get(), 0);
        
        result = variable.compute(store, now.plus(Duration.ofSeconds(180)));
        assertTrue(result.getAsFloat().isPresent());
        assertEquals(10, result.getAsFloat().get(), 0);
        
        result = variable.compute(store, now.plus(Duration.ofSeconds(181)));
        assertTrue(result.getAsFloat().isPresent());
        assertEquals(20, result.getAsFloat().get(), 0);
    }
    ...
    public void ignorePeriodInAggregation() throws ConfigurationException, ComputationException {
        ...
        assertResult(0, 0f, 0);
        assertResult(0, 0f, 20);
        assertResult(0, 0f, 40);
        assertResult(0, 0f, 60);
        assertResult(1, 0f, 61);
        assertResult(1, 0f, 80);
        assertResult(2, 0f, 100);
        assertResult(3, 0f, 120);
    }
    ...
    public void weightedAverageAggreagtion() throws ConfigurationException, ComputationException {
        ...
        weight1 = (float) (period.toMillis() - Duration.between(metric2_time, metric1_time).abs().toMillis()) / period.toMillis();
        float weight2 = (float) (period.toMillis() - Duration.between(metric2_time, metric2_time).abs().toMillis()) / period.toMillis();
        ...
        weight1 = (float) (period.toMillis() - Duration.between(metric3_time, metric1_time).abs().toMillis()) / period.toMillis();
        weight2 = (float) (period.toMillis() - Duration.between(metric3_time, metric2_time).abs().toMillis()) / period.toMillis();
        float weight3 = (float) (period.toMillis() - Duration.between(now, metric3_time).abs().toMillis()) / period.toMillis();
        expectedValue = (metric1_value * weight1 + metric2_value * weight2 + metric3_value * weight3) / (weight1 + weight2 + weight3);
    ...
    private void assertResult(float expected, float newValue, int nowPlusSeconds) {
        ...
        variable.updateStore(store, new Metric(now.plus(Duration.ofSeconds(nowPlusSeconds)), new FloatValue(newValue), ids));
