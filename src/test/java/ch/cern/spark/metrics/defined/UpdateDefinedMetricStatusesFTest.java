...
public class UpdateDefinedMetricStatusesFTest {
    ...
    public void shouldGenerateWhenUpdatingVariable() throws Exception {
        ...
        props.get().setProperty("metrics.define.dmID1.variables.value.expire", "10s");
    ...
    public void shouldAggregateWhenGroupByIncludeAllAttributes() throws Exception {
        PropertiesCache props = PropertiesTest.mockedExpirable();
        props.get().setProperty("metrics.define.dmID1.metrics.groupby", "INSTANCE_NAME");
        props.get().setProperty("metrics.define.dmID1.variables.value.aggregate", "count");
        props.get().setProperty("metrics.define.dmID1.variables.value.expire", "5s");
        DefinedMetrics definedMetrics = new DefinedMetrics(props);
